...
const DetailEvent = () => {
  ...
  return (
    <div className="detail-event h-[1024px] pt-10 mx-4 lg:mx-20">
      <div className={`detail-event-container ${isLoaded ? "loaded" : ""} ${isExiting ? "exiting" : ""}`}>
        ...
        <div className="content-box flex flex-col md:flex-row">
          ...
          <div className="description text-left mx-8 mt-4 md:mt-0 md:ml-8">
            ...
            <h1 className="font-bold text-[32px] py-4 sm:text-[24px]">{eventData.judul}</h1>
            ...
            <div className="flex gap-2 ml-2">
              ...
              <span className="font-medium text-[16px] sm:text-[14px] mt-2 ml-auto mr-2">{eventData.start_time} - {eventData.end_time}</span>
            </div>
            <div className="flex gap-2 ml-1 my-4">
              <i className="ri-map-pin-2-fill text-4xl sm:text-3xl"></i>
              <span className="font-medium text-[16px] sm:text-[14px] mt-2">{eventData.tempat}</span>
              <img src={Chair} alt="Location" className="text-4xl sm:text-3xl ml-auto" />
              <span className="font-medium text-[16px] sm:text-[14px] mt-2 mr-2">{eventData.available_slot} Kursi</span>
            </div>
            <div className="border-b-2 border-[#003266] w-full my-4"></div>
            <div className="lecturer flex gap-2 ml-2 w-auto">
              <img src={eventData.foto_pembicara || Lecturer} alt="Profile" className="w-16 h-16 text-4xl sm:text-3xl rounded-full" />
              <div className="lecturername flex flex-col ml-4 gap-2 justify-center">
                <span className="font-semibold text-[16px] sm:text-[14px]">{eventData.pembicara}</span>
                <span className="text-regular text-[14px] sm:text-[12px]">{eventData.role}</span>
              </div>
            </div>
            <div className="border-b-2 border-[#003266] w-full my-4"></div>
            <div>
          ...
          <div className="booking w-full md:w-4/12 h-36 px-6 mx-auto bg-white shadow-lg rounded-2xl flex flex-col mt-4 md:mt-0">
            <h1 className="text-left my-4 font-semibold text-[20px] sm:text-[18px] pl-2 lg:text-left sm:text-center ">Pesan Sekarang!</h1>
            <button
              className="bg-[#027FFF] font-regular w-full h-11 my-4 rounded-lg text-medium text-white text-[16px] sm:text-[14px]"
              onClick={() => navigate(`/previewevent/${eventData.id}`)}
            >
              Pesan
            </button>
          </div>
